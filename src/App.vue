...
type ClientConfig = {
  ...
  adb_enabled: boolean;
  adb_device_discovery_enabled: boolean;
...
type ClientStatus = {
  config: ClientConfig;
  registered: boolean;
  enrollment_credential_available: boolean;
...
const overviewWorkflowPreviews = ref<WorkflowProcessPreview[]>([]);
const serverDomain = ref("https://factory.follow-flow.de");
const enrollmentCredential = ref("");
const adbEnabled = ref(true);
const adbDeviceDiscoveryEnabled = ref(true);
...
const online = computed(() => Boolean(status.value?.registered));
const canRegister = computed(() => Boolean(!status.value?.registered && status.value?.enrollment_credential_available));
const runtimeReady = computed(() => Boolean(status.value?.node_available && status.value?.workflow_runtime_available));
const canDiscoverDevices = computed(() => Boolean(status.value?.config.adb_enabled && status.value?.config.adb_device_discovery_enabled));
const activeWorkflowProcesses = computed(() => processes.value.filter((process) => isWorkflowProcess(process) && process.status === "running").slice(0, 6));
...
async function refresh(silent = false) {
  try {
    ...
    overviewWorkflowPreviews.value = await loadOverviewWorkflowPreviews(localProcesses);
    ...
    adbEnabled.value = clientStatus.config.adb_enabled;
    adbDeviceDiscoveryEnabled.value = clientStatus.config.adb_device_discovery_enabled;
...
async function discoverDevices() {
  if (!canDiscoverDevices.value) {
    setFeedback("ADB-Gerätesuche ist lokal deaktiviert.", "info");
    return;
  }
...
async function saveEnrollmentCredential() {
  const credential = enrollmentCredential.value.trim();
  if (!credential) {
    setFeedback("Bitte zuerst einen einmaligen Enrollment-Schlüssel eingeben.", "error");
    return;
  }

  await runAction("enrollment", async () => {
    await invoke("configure_bootstrap_credential", { bootstrapApiKey: credential });
    enrollmentCredential.value = "";
    await refresh(true);
    setFeedback("Enrollment-Schlüssel wurde sicher im Windows Credential Manager gespeichert.", "success");
  });
}
...
function normalizeAdbSettings() {
  if (!adbEnabled.value) adbDeviceDiscoveryEnabled.value = false;
}

async function saveAdbSettings() {
  normalizeAdbSettings();
  await runAction("adb-settings", async () => {
    await invoke("update_adb_settings", {
      settings: {
        adb_enabled: adbEnabled.value,
        adb_device_discovery_enabled: adbDeviceDiscoveryEnabled.value,
      },
    });
    await refresh(true);
    setFeedback("ADB-Einstellungen wurden lokal gespeichert.", "success");
  });
}
...
async function loadOverviewWorkflowPreviews(sourceProcesses: LocalProcess[]) {
  const jobs = sourceProcesses
    .filter((process) => isWorkflowProcess(process) && process.status === "running" && process.job_id)
    .slice(0, 6);
  const previews = await Promise.all(jobs.map(async (process) => {
    try {
      return await invoke<WorkflowProcessPreview>("get_workflow_process_preview", { jobUuid: process.job_id });
    } catch {
      return null;
    }
  }));

  return previews.filter((preview): preview is WorkflowProcessPreview => Boolean(preview));
}

function workflowPreviewTitle(preview: WorkflowProcessPreview) {
  return stringValue(preview.status, ["workflowStepName", "currentStepName"], preview.job_uuid);
}

function workflowPreviewState(preview: WorkflowProcessPreview) {
  return stringValue(preview.status, ["state", "status"], "running");
}

function workflowPreviewMessage(preview: WorkflowProcessPreview) {
  return stringValue(preview.status, ["message", "statusMessage", "stage"], "Workflow läuft.");
}

async function openWorkflowPreviewByJob(jobUuid: string) {
  await runAction(`preview-${jobUuid}`, async () => {
    workflowPreviewJob.value = jobUuid;
    workflowPreview.value = await invoke<WorkflowProcessPreview>("get_workflow_process_preview", { jobUuid });
  });
}
    ...
    <main class="workspace">
      ...
      <template v-if="activePage === 'overview'">
        ...
        <section class="stats-grid">
          ...
          <article class="metric"><span>Lokale Geräte</span><strong>{{ status?.local_devices ?? 0 }}</strong><small>{{ status?.config.adb_enabled ? (status?.adb_available ? `ADB: ${status.adb_source}` : "ADB nicht verfügbar") : "ADB deaktiviert" }}</small></article>
        ...
        <section v-if="overviewWorkflowPreviews.length" class="panel overview-process-panel">
          <div class="panel-heading"><div><h2>Aktive Workflow-Prozesse</h2><p>{{ activeWorkflowProcesses.length }} laufende(r) Prozess(e), maximal sechs Vorschaukarten.</p></div><span class="live-indicator"><i></i> Live</span></div>
          <div class="overview-process-grid">
            <article v-for="preview in overviewWorkflowPreviews" :key="preview.job_uuid" class="overview-process-card" @click="openWorkflowPreviewByJob(preview.job_uuid)">
              <div class="overview-shot">
                <img v-if="preview.screenshot_data_url" :src="preview.screenshot_data_url" alt="">
                <span v-else>Kein Screenshot</span>
              </div>
              <div class="overview-process-body">
                <div><strong>{{ workflowPreviewTitle(preview) }}</strong><span :class="['pill', workflowPreviewState(preview)]">{{ workflowPreviewState(preview) }}</span></div>
                <p>{{ workflowPreviewMessage(preview) }}</p>
                <code>{{ preview.job_uuid }}</code>
              </div>
            </article>
          </div>
        </section>
        ...
        <section class="panel">
          ...
          <div class="action-grid"><button class="action-card" @click="registerNode" :disabled="isBusy || !canRegister"><span>01</span><strong>Node registrieren</strong><small>{{ status?.registered ? "Node ist bereits registriert" : status?.enrollment_credential_available ? "Einmaligen Schlüssel gegen Node-Schlüssel tauschen" : "Enrollment-Schlüssel unter Einstellungen hinterlegen" }}</small></button><button class="action-card" @click="sendHeartbeat" :disabled="isBusy || !online"><span>02</span><strong>Heartbeat senden</strong><small>Version und Fähigkeiten melden</small></button><button class="action-card" @click="discoverDevices" :disabled="isBusy || !canDiscoverDevices"><span>03</span><strong>Geräte erkennen</strong><small>{{ canDiscoverDevices ? "Lokalen ADB-Scan starten" : "ADB-Suche deaktiviert" }}</small></button><button class="action-card" @click="syncDevices" :disabled="isBusy || !canDiscoverDevices || !online"><span>04</span><strong>Geräte synchronisieren</strong><small>{{ canDiscoverDevices ? "Inventar an AiUserFactory senden" : "ADB-Suche deaktiviert" }}</small></button></div>
        </section>
      </template>

      <section v-else-if="activePage === 'devices'" class="panel">
        <div class="panel-heading"><div><h2>Android-Geräte</h2><p>Direkt am Node erkanntes Geräte-Inventar.</p></div><div class="top-actions"><button class="button ghost" @click="discoverDevices" :disabled="isBusy || !canDiscoverDevices">ADB-Scan</button><button class="button primary" @click="syncDevices" :disabled="isBusy || !canDiscoverDevices || !online">Zum Server syncen</button></div></div>
      ...
      <section v-else class="settings-grid">
        ...
        <article class="panel"><div class="panel-heading"><div><h2>Sicheres Enrollment</h2><p>Einmaligen Schlüssel lokal im Windows Credential Manager hinterlegen.</p></div><span :class="['pill', status?.registered ? 'success' : 'waiting']">{{ status?.registered ? "Registriert" : status?.enrollment_credential_available ? "Bereit" : "Schlüssel fehlt" }}</span></div><label for="enrollment-credential">Einmaliger Enrollment-Schlüssel</label><input id="enrollment-credential" v-model="enrollmentCredential" type="password" autocomplete="off" spellcheck="false" :disabled="isBusy || status?.registered" placeholder="Mindestens 32 Zeichen"><p class="security-note">Der Schlüssel wird nicht in client.json gespeichert und nach erfolgreicher Registrierung automatisch gelöscht.</p><button class="button primary" @click="saveEnrollmentCredential" :disabled="isBusy || status?.registered || !enrollmentCredential.trim()">Sicher speichern</button></article>
        <article class="panel"><div class="panel-heading"><div><h2>ADB</h2><p>Lokale Android-Steuerung und Gerätesuche.</p></div></div><div class="toggle-list"><label class="toggle-row"><input type="checkbox" v-model="adbEnabled" @change="normalizeAdbSettings"><span><strong>ADB-Steuerung aktiv</strong><small>Erlaubt lokale ADB-Kommandos auf diesem Controller.</small></span></label><label class="toggle-row"><input type="checkbox" v-model="adbDeviceDiscoveryEnabled" :disabled="!adbEnabled"><span><strong>Gerätesuche aktiv</strong><small>Erlaubt automatische und manuelle ADB-Gerätescans.</small></span></label></div><button class="button primary" @click="saveAdbSettings" :disabled="isBusy">ADB speichern</button></article>
